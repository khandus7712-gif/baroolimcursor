...
아래 입력값을 바탕으로 **900~1,500자** 분량의 네이버 블로그 글을 생성하라.
글이 1,500자를 넘지 않도록 내용을 압축하고, 불필요한 반복/군더더기를 제거하라.`);
...
**7. 분량:** 전체 글은 **900~1,500자** 사이로 작성한다. 1,500자를 넘지 마라.
...
function createThreadsPrompt(
  ...
  const sections: string[] = [];
  const hookByDomain: Record<string, string> = {
    food: '첫 줄은 식재료 희소성 또는 가족외식 공감형으로 시작한다. 예: "소 한 마리에서 4kg, 창원엔 여기뿐이에요 🔥", "가족 외식 고민 끝났어요 😊", "월요일만 쉬어요. 그만큼 매일 준비해요"',
    cafe: '첫 줄은 공간·분위기 공감형으로 시작한다. 예: "카페 고르다 지치셨죠? 여기 오세요"',
    beauty: '첫 줄은 변화 전/후 공감형으로 시작한다. 예: "머리 자르고 싶은데 어디 가야 할지 몰랐죠?"',
    retail: '첫 줄은 희소성·가격 메리트형으로 시작한다. 예: "이 가격에 이 퀄리티, 오프라인에서만 가능해요"',
    fitness: '첫 줄은 동기부여 공감형으로 시작한다. 예: "작심삼일 반복되시죠? 저도 그랬어요"',
    pet: '첫 줄은 반려동물 걱정 공감형으로 시작한다. 예: "우리 아이 먹거리, 진짜 고민 많으시죠?"',
    education: '첫 줄은 학부모·학생 고민 공감형으로 시작한다. 예: "어느 학원 보내야 할지 모르겠다는 말, 저도 들었어요"',
  };

  sections.push(`[CONTENT]

아래 입력값을 바탕으로 **Threads 게시글 1세트(본문 + 댓글 1개)**를 생성하라.`);
...
[본문]
첫 줄 (15자 이내, 강한 후킹)
2~5줄 (핵심 내용, 이모지 1~2개만)
마지막 줄 (예약/방문 유도)

[댓글]
주소 | 영업시간 | 전화번호 | 대표메뉴+가격
네이버 지도 링크 (있을 경우)

### ◆ 공통 규칙 (모든 업종)
- 댓글은 1개만 출력한다.
- 본문은 5줄 이내로 작성한다.
- 마크다운 기호(#, *, -)는 절대 출력하지 않는다.
- 해시태그를 본문에 넣지 않는다.
- "~했어요", "~더라고요" 구어체를 사용한다.
- 이모지는 본문에 1~2개만 사용한다.
- 과장 표현 금지, 사실 기반으로만 작성한다.
- 금지 단어: "정체성", "진가", "기초", "신뢰", "성실한"

### ◆ 업종별 첫 줄 후킹 (업종 카테고리 기준 분기)
- 현재 업종(${domain.id}) 첫 줄 규칙: ${hookByDomain[domain.id] || '공감형 후킹으로 시작한다.'}
...
function createInstagramPrompt(
  ...
  const instagramHookRules: Record<string, string> = {
    food: '첫 줄은 식재료 희소성 또는 공감형으로 시작한다. 예: "가족 외식 메뉴, 오늘도 고민되시죠?"',
    cafe: '첫 줄은 공간·분위기 공감형으로 시작한다. 예: "카페 고르다 지치셨죠?"',
    beauty: '첫 줄은 변화 전/후 공감형으로 시작한다. 예: "머리 자르고 싶은데 어디 가야 할지 고민되시죠?"',
    retail: '첫 줄은 제품 희소성 또는 가격 메리트 강조형으로 시작한다.',
    fitness: '첫 줄은 동기부여 공감형으로 시작한다. 예: "작심삼일, 또 반복되고 있진 않으세요?"',
    pet: '첫 줄은 반려동물 걱정 공감형으로 시작한다. 예: "우리 아이 먹거리, 매번 고민되시죠?"',
    education: '첫 줄은 학부모/학생 고민 공감형으로 시작한다. 예: "어느 학원을 보내야 할지 고민되시죠?"',
  };
  const firstHookRule = instagramHookRules[domain.id] || '첫 줄은 공감형 후킹으로 시작한다.';
...
**0) 공통 규칙 (반드시 준수)**
- 문체는 자연스러운 구어체로 작성한다. ("~했어요", "~더라고요" 적극 사용)
- 한 줄씩 끊어서 가독성 높게 작성한다. (긴 문단 금지)
- 과장 표현 금지, 사실 기반 정보만 작성한다.
- 가격 정보는 반드시 포함한다.
- 마지막 줄에는 반드시 주소·영업시간·전화번호를 포함한다.
- 브로슈어/홍보물 문체(딱딱한 나열형) 금지.

**1) 강력한 2줄 후킹으로 시작 (업종별 분기 필수)**
- 짧고 명확하며 감각적이어야 한다.
...
- 예: "👉 자세히 보기: https://XXXX"
- 행동을 부드럽게 유도하는 문장 포함
- 단, 본문 마지막 안내 줄(주소·영업시간·전화번호)은 CTA 다음에 반드시 포함
