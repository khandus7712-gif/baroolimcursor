...
function createContentSection(
  ...
  if (platform.id === 'blog') {
    sections.push(
      `\n**블로그 콘텐츠 필수 요구사항:**\n1. **모든 내용은 반드시 한국어로만 작성하세요. 영어 단어, 문장, 표현, 괄호 안의 영어 번역을 절대 사용하지 마세요.**\n2. **최소 1500자 이상 작성 필수** (공백 제외)\n3. **제목(H1)은 '메뉴/서비스 + 상황 + 지역 + 강력 키워드' 중 최소 2개를 조합**하여 작성하세요 (예: "창원 연말 회식은 전골+육전 조합이 답입니다").\n4. **첫 문단은 계절·문제·트렌드 기반 Hook**으로 시작해 읽을 이유를 만드세요 (예: "연말 회식은 메뉴 실패하면 분위기 망합니다.").\n5. **USP(장점)는 글 상단 30% 이내에 3회 이상, 서로 다른 문장으로 반복 노출**하세요 (예: "매일 삶아 깊은 국물", "위생 관리 철저", "단체 예약도 여유로운 좌석").\n6. **단골 Pain Point 최소 1개 포함** (예: "단체로 가면 양이 부족할까 걱정하시죠?").\n7. **감각 묘사(냄새·온도·식감) 최소 2회 포함** — 전골의 김, 향, 온도 등을 생생하게 묘사하세요.\n8. **CTA는 즉시성/희소성/편의성 중 하나 이상을 포함**하여 작성하세요 (예: "점심 회식은 자리 빨리 찹니다. 지금 예약하세요.").\n9. 모든 단락 사이에 빈 줄(\\n\\n) 삽입, 소제목(##) 전후로 빈 줄 2개씩 삽입, 각 단락은 2-4문장으로 간결하게 구성하세요.\n10. 긴 텍스트 블록 금지, 정보형 + 설득형의 균형을 유지하고, 중복 문장은 허용되지 않습니다.\n11. 해시태그 10~15개는 메뉴 + 지역 + 상황 중심으로만 작성하며, 의미 없는 일반 태그(#맛있는, #카페 등)는 금지입니다.\n12. **섹션 제목은 '도입부', '본문', '결론' 같은 일반적인 단어 대신 내용에 맞는 자연스러운 제목**을 사용하세요 (예: '아롱하다의 특별한 메뉴', '맛의 비밀', '위치 및 예약 안내').\n13. **이미지 설명을 본문에 포함할 때는 'Caption:', '**Caption:**', '이미지 설명:' 같은 라벨을 사용하지 말고, 자연스러운 문장으로 이미지 내용을 설명하세요.**`
    );
  }
  ...
  if (platform.id === 'instagram') {
    sections.push(
      `\n**인스타그램 필수 규칙:**\n1. 첫 문장은 반드시 강력한 Hook으로 시작하세요 (HOT, PAIN, SCARCITY 기반).\n2. 두 번째 단락에서 고객 고민(Pain Point)을 명확히 언급하세요.\n3. USP(브랜드 강점)는 본문 상단에 배치하세요 (예: 매일 삶아서 깊은 맛, 위생 관리 철저 등).\n4. 동일 문장/의미 반복 금지. 반복될 경우 다른 표현으로 재작성하세요.\n5. 문장은 5~13 단어 내로 짧고 리듬감 있게 작성하고, 단락은 1~2줄로 유지하세요.\n6. 해시태그는 업종 + 지역 + 메뉴 + 상황 중심으로 10~15개 작성하고, 의미 없는 일반 태그(#맛있는, #카페 등)는 사용하지 마세요.\n7. CTA는 반드시 즉시성 또는 희소성을 포함하세요 (예: \"오늘도 예약 금방 마감돼요. 지금 연락 주세요!\").`
    );
  }

  if (platform.id === 'threads') {
    sections.push(
      `\n**Threads 필수 규칙:**\n1. 첫 줄은 반드시 강력한 Hook으로 시작하세요 (문제 제기형, 위험 알림형, 궁금증 유발형 중 하나).\n2. 전체 글은 3~4줄로 제한하고, 각 줄은 하나의 메시지만 전달하세요. 긴 문단/설명식 문장은 금지입니다.\n3. 두 번째 줄에는 고객의 Pain Point를 포함하세요 (예: \"단체로 가면 양·맛·위생 때문에 고민 많잖아요.\").\n4. 세 번째 줄에는 USP(핵심 장점)를 넣으세요 (예: \"매일 삶는 전골\", \"육수·냄비까지 깨끗 관리\", \"겉바속촉 육전\").\n5. 네 번째 줄은 강력한 CTA로 마무리하고 즉시성을 포함하세요 (예: \"오늘 점심은 아롱하다에서 따뜻하게 드세요! 지금 예약!\").\n6. 해시태그는 업종 + 지역 + 메뉴 중심으로 3~6개만 작성하며, 의미 없는 일반 태그(#foodie, #맛있는 등)는 사용하지 마세요.\n7. 이모지는 줄당 1~2개 이하로 제한하세요.`
    );
  }
